...
- **手動修改**：
  ...
  - 生詞卡可直接改「詞語」：自動帶出讀音（破音字）、拼音，找得到時連英文一起帶入，之後都可再微調。
    查找順序是「任一章生詞卡已有的詞 → 課文中相同的連續字 → 逐字從課文找讀音」，目前這章優先；
    學而篇沒出現過的字會提示手動補拼音。
- **即時預覽**：右側逐頁預覽，所見即所得。
- **下載 PPT**：一鍵產生 `.pptx`，版面與預覽一致。
- **自動存草稿**：每次修改都會自動存在這台電腦的瀏覽器裡，重新整理或關掉再開都還在（章節選單會標示「（有修改）」）。
- **下載 YAML**：把校對結果存成與 `examples/v2/*.yaml` 相同格式的檔案，用來永久更新預設值（見下方「校對後更新預設值」）。
- **自動調整版面**：英文過長會換行並預留行距；一頁放不下時整頁等比例縮小，最後一行不會壓到頁尾。

### 校對後更新預設值

草稿只存在**目前這個瀏覽器**（換電腦、清除瀏覽資料就不見了），確定校對完成後請寫回 YAML：

1. 按 **下載 YAML**，得到例如 `學而1.6.yaml`。
2. 用它覆蓋 repo 裡的 `examples/v2/學而1.6.yaml`，用 `git diff` 確認只改了你校對的地方。
3. commit 並 push → GitHub Actions 自動重建網站，新內容就成為所有人看到的預設值；命令列版 `make_analects_deck.py` 讀的也是同一份 YAML。
4. 網站更新後再開這一章，草稿與新預設相同會自動清掉。
...
| 加上版本號 | 把 `index.html`、`styles.css` 裡的 `__BUILD__` 換成 commit 編號，所有檔案網址變成 `?v=xxxxxxx`，瀏覽器每次部署都會抓新檔、不會混用新舊版本 |
...
│   ├── app.js         # 編輯器主邏輯（含自動存草稿）
│   ├── yaml.js        # 匯出與 examples/v2 相同格式的 YAML
│   ├── vocab.js       # 生詞卡：輸入詞語自動帶出讀音／拼音／英文
│   └── pptxgen.bundle.js
